...
#define DEV_PATH "/dev/ttySAC1"   //COM2
...
int PiccRequest(int fd)
{
	...
	memset(WBuf, 0, 128);//清空  bzero();
	memset(RBuf,0,128);
	WBuf[0] = 0x07;	//帧长= 7 Byte
	WBuf[1] = 0x02;	//包号= 0 , 命令类型= 0x01
	WBuf[2] = 0x41;	//命令= 'A'
	WBuf[3] = 0x01;	//信息长度= 1
	WBuf[4] = 0x52;	//请求模式:  ALL=0x52  DLE：待机模式
	...
	tcflush (fd, TCIFLUSH);
	write(fd, WBuf, 7);
	//你如何得知读卡器是否给你应答，---》通过select去监测串口的文件描述符看是否可读
...
int main(int argc,char *argv[])
{
	...
	if (fd < 0)
	{
		fprintf(stderr, "Open Gec6818_ttySAC1 COM2 fail!\n");
		return -1;
	}
	/*初始化串口*/
	init_tty(fd);
