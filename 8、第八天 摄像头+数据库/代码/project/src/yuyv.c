...
int do_camera()
{
	...
	while(1)
	{
		...
		if((x>0&&x<80)&&(y>0&&y<480))//抓拍
		{
			int zp_fd = open("./0.jpg",O_RDWR | O_CREAT | O_TRUNC,0644);
			if(zp_fd < 0)
			{
				printf("open file failed\n");
				return -1;
			}
			write(zp_fd,jpg_buf.jpg_data,jpg_buf.jpg_size);
			close(zp_fd);
			system("tftp 192.168.16.xx -p -l 0.jpg");
		}
		if((x>720&&x<800)&&(y>0&&y<480))//退出
		{
			break;
		}
	}
