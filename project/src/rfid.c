...
extern time_t tim;
int time_min1,time_sec1,time_min2,time_sec2,time_sec;
int consume_sum;  //停车的费用
int car_sum = 0;
/*串口(RFID模块)初始化 */
...
int Rfid_Recharge(void)
{
	...
	while(1)
	{
		...
		//确认充值和充值界面
		lcd_draw_jpg(0,0,"../pictures/background_charge.jpg",NULL,0,0);
		lcd_draw_jpg(0,0,"../pictures/stop.jpg",NULL,0,0);
		//lcd_draw_jpg(0,430,"../pictures/quit.jpg",NULL,0,0);
		//充值金额图片显示
		lcd_draw_jpg(80,180,"../pictures/charge_10.jpg",NULL,0,0);
		lcd_draw_jpg(300,180,"../pictures/charge_20.jpg",NULL,0,0);
		lcd_draw_jpg(520,180,"../pictures/charge_50.jpg",NULL,0,0);
		lcd_draw_jpg(80,300,"../pictures/charge_100.jpg",NULL,0,0);
		lcd_draw_jpg(300,300,"../pictures/charge_200.jpg",NULL,0,0);
		lcd_draw_jpg(520,300,"../pictures/charge_500.jpg",NULL,0,0);
		
		while(1)
		{
			if((x>80&&x<280)&&(y>180&&y<280))//6个金额充值的按钮  10  的坐标
			{
				x=0;
				y=0;
				Recharge_amount = 10;
				lcd_draw_jpg(200,140,"../pictures/charge_verify.jpg",NULL,0,0);//显示的是 是否确认充值   小图标
				while(1)
				{
					if((x>200&&x<320)&&(y>340&&y<420))//点击确认充值
					{
						//char balance[20] = {0};
						//sprintf(balance,"sum    %d yuan",money);
						//Clean_Area(300,200,200,150,0x0000ff00);
						//Display_characterX(300,100,balance,0x00ff0000,3);
						lcd_draw_jpg(250,200,"../pictures/charge_sccess.jpg",NULL,0,0);
						//sleep(1.5);
						break;
					}
					if((x>480&&x<600)&&(y>340&&y<420))//点击了“否”
					{
						Recharge_amount = 0;
						goto loop1;
					}
					if((x<100&&x>0)&&(y<480&&y>430))       
					{
						Recharge_amount = 0;
						goto loop1;
					}
			...
			//*if()//4个金额充值的按钮  20 的坐标
			if((x>300&&x<500)&&(y>180&&y<280))//6个金额充值的按钮  20  的坐标
			{
				x=0;
				y=0;
				Recharge_amount = 20;
			...
			//4个金额充值的按钮  50  的坐标
			if((x>520&&x<720)&&(y>180&&y<280))//6个金额充值的按钮  50  的坐标
			{
				x=0;
				y=0;
				Recharge_amount = 50;
			...
			if((x>80&&x<280)&&(y>300&&y<400))               //6个金额充值的按钮  100  的坐标
			{
				x=0;
				y=0;
				Recharge_amount = 100;
			...
			if((x>300&&x<500)&&(y>300&&y<400))//6个金额充值的按钮  200  的坐标
			{
				x=0;
				y=0;
				Recharge_amount = 200;
			...
			//4个金额充值的按钮  500  的坐标
			if((x>520&&x<720)&&(y>300&&y<400))//6个金额充值的按钮  500  的坐标
			{
				x=0;
				y=0;
				Recharge_amount = 500;
...
loop1:
	close(fd);
	//显示主界面
	lcd_draw_jpg(0,0,"../pictures/background1.jpg",NULL,0,0);
	lcd_draw_jpg(0,0,"../pictures/stop.jpg",NULL,0,0);
...
int Rfid_Consumption(void)
{
	...
	//显示消费界面
	lcd_draw_jpg(0,0,"../pictures/background2.jpg",NULL,0,0);
	lcd_draw_jpg(0,0,"../pictures/stop.jpg",NULL,0,0);
	//lcd_draw_jpg(0,0,"../pictures/background1.jpg",NULL,0,0);
	lcd_draw_jpg(700,0,"../pictures/quit.jpg",NULL,0,0);
	...
	while(1)
	{
		...
		//读取卡的余额
		money = DataReadBuf[0]*100+DataReadBuf[1];
		printf("当前余额为：%d元/n",money);
		if(flag/2 == 1)
		{
			//记录分1和秒1；
			time_min1 = timeinfo->tm_min;
			time_sec1 = timeinfo->tm_sec;
			car_sum ++;
		}
		if(flag/2==0)
		{
			//记录分2和秒2；
			time_min2 = timeinfo->tm_min;
			time_sec2 = timeinfo->tm_sec;
			car_sum --;
			//计算入库的时间
			time_sec = (time_min2 - time_min1)*60 + (time_sec2 - time_sec1);
			//计算停车的费用
			consume_sum = time_sec;
			//consume_sum *= 10;
			//余额不知以扣除消费金额的情况
			printf("consume_sum:%d元\n",consume_sum);
			if(money<consume_sum)
			{
				char balance[50] = {0};
				sprintf(balance,"sum is lack",money);
				Display_characterX(100,100,balance,0x00ff0000,3);
				sleep(2);
				Rfid_Recharge();
			}
			//计算当前余额
			money = money - consume_sum;
			sleep(0.5);
			//写卡
			if(PiccWrite(fd,SECTOR))
			{
...
void Rfid_find()
{
	lcd_draw_jpg(0,0,"../pictures/12.jpg",NULL,0,0);	
	sleep(3);
}
